...
|:---|:---|:---|
| `dopamine` | `dobutamine` | Vasopressors — very similar names & routes |
...
| 🧮 **9-Feature Engineering** | Computes Levenshtein distance, Jaro-Winkler, Soundex, Metaphone, Bi/Trigram overlap, prefix match, length ratio |
| 🤖 **ML Classification** | Ensemble model (Random Forest + optional XGBoost) trained on balanced positive/negative drug pairs |
| 🗣️ **NLP Drug Extraction** | Regex + curated dictionary-based clinical NER  — extracts drug name, dosage, route from free-text sentences |
| 🎙️ **Speech-to-Text** | OpenAI Whisper integration for voice-based clinical input with confidence scoring |
| 🏥 **Patient Context Validation** | Secondary safety layer — cross-references drug class against patient diagnosis taxonomy |
| ⚖️ **Decision Engine** | Context-aware risk stratification (🟢 LOW / 🟡 MEDIUM / 🔴 HIGH) with human-readable reasons |
...
└─────────────────────┬───────────────────────────────────────────────┘
          ...
          ┌───────────▼───────────┐
          │  Module 06            │
          │  Speech-to-Text       │  ← OpenAI Whisper (or fallback mock)
          │  (Transcription)      │    Returns: { text, confidence }
          └───────────┬───────────┘
                      │
          ┌───────────▼───────────┐
          │  Module 05            │
          ...
          ┌───────────▼───────────┐
          │  Module 09 (app.py)   │
          │  FastAPI Web Server   │  ← REST endpoints + HTML frontend
          └───────────────────────┘
```
...
(ISMP data)   (Pairs)       (Features)    (Training)     (Saved Model)
```
...
│   ├── processed/
│   │   ├── drug_pairs.csv           # Extracted positive LASA pairs
│   │   ├── training_dataset.csv     # Balanced labeled dataset (pos + neg)
│   │   └── feature_matrix.csv       # 9-dimensional feature vectors per pair
│   └── drug_list.txt                # Master drug name list (~300+ entries)
│
├── modules/
...
│   ├── 03_model_training.py         # Random Forest training + evaluation
...
│   └── modules_utils.py             # Shared feature function (used by engine + API)
│
├── models/
│   └── lasa_classifier.pkl          # Serialized trained model (joblib)
│
├── app/
...
├── bootstrap.py                     # Auto-creates directory structure
├── run_all.py                       # Full sequential pipeline runner
...
- Windows / macOS / Linux
- (~500 MB disk space if using Whisper voice input)
...
This will automatically: create a virtual environment → install all dependencies → run the training pipeline → start the web server at `http://localhost:8001`.
...
# 3. (Optional) Add your ISMP PDFs to data/raw/ and run preprocessing
...
Navigate to `http://localhost:8001/` and fill in the form:
...
> **Why LOW despite 99% similarity?** The model found that `metformin` and `methergine` are phonetically similar, but because `metformin` (an antidiabetic) is validated for the `diabetes` diagnosis with no mismatch, the decision engine correctly suppresses the alert. This prevents *alert fatigue*.
...
> **Why HIGH?** The NLP extracted `hydralazine`. The LASA engine found `hydroxyzine` at 97% similarity (a known ISMP pair). Crucially, the Patient Context Validator flagged that `hydralazine` is an **antihypertensive** — not an anxiety treatment — triggering the full HIGH risk alert and dual-reason output.
...
The system will transcribe the audio using **OpenAI Whisper**, extract the drug name, and return a full risk analysis including the transcription confidence score.
...
The decision engine uses **context-aware, multi-signal risk stratification** to prevent alert fatigue:

| Risk Level | Trigger Conditions |
|:---|:---|
| 🔴 **HIGH** | LASA similarity > 0.75 **AND** patient diagnosis mismatch detected |
| 🟡 **MEDIUM** | Drug is a known ISMP pair (explicit historical report) with similarity > 0.80, or low STT audio confidence |
| 🟢 **LOW** | Drug-diagnosis context is validated, or no significant LASA similarity found |

> **Key Design Decision:** A drug can score 99% LASA similarity and still return **LOW risk** if the prescribed drug is clinically appropriate for the patient's diagnosed condition. This mirrors how a real pharmacist would reason — flagging only *actionable* risks.

---

## 📊 Model Performance

The Random Forest classifier was evaluated on a balanced held-out test set:
...
1. `levenshtein_norm` — Normalized edit distance
2. `jaro_winkler` — Winkler-adjusted character alignment score
...
|:---|:---|:---|
| `scikit-learn` | 1.x | Random Forest classifier, train/test pipeline |
| `xgboost` | 2.x | Optional gradient boosting (alternate model) |
| `pandas` | 2.x | Tabular data manipulation, feature matrix |
...
| Library | Version | Role |
|:---|:---|:---|
| `rapidfuzz` | 3.x | Levenshtein distance, Jaro-Winkler, token sort ratio |
...
| `openai-whisper` | 1.x | Speech-to-text transcription (voice input) |

### Web & Serving

| Library | Version | Role |
|:---|:---|:---|
| `fastapi` | 0.135 | REST API framework with automatic OpenAPI docs |
| `uvicorn` | 0.x | ASGI production server |
| `jinja2` | 3.x | HTML template rendering |
| `python-multipart` | 0.x | Multipart form / file upload parsing |

### Visualization (Training Phase)

| Library | Version | Role |
|:---|:---|:---|
| `matplotlib` | 3.x | AUC-ROC curve, feature importance plots |
| `seaborn` | 0.x | Correlation heatmaps, confusion matrix |

---

## 🧬 Feature Engineering Details

Nine similarity features are computed for every drug pair `(A, B)`:
...
| `soundex_match` | `int(soundex(A) == soundex(B))` | 0 or 1 |
| `metaphone_match` | `int(metaphone(A) == metaphone(B))` | 0 or 1 |
...
The patient context validator currently maps these diagnoses to expected drug classes:
...
| `heart_failure` | inotropic, diuretic |

---

## ⚙️ Configuration

No configuration file is required. Key paths are resolved relative to the project root automatically. To override:

```python
# In modules/04_lasa_engine.py
DRUG_LIST_PATH = "data/drug_list.txt"     # Master drug list
MODEL_PATH     = "models/lasa_classifier.pkl"  # Trained model artifact
```
...
- **ISMP (2023)** — *ISMP List of Confused Drug Names* — Primary dataset source
- **Cohen MR (1999)** — *Medication Errors* — Foundational taxonomy of LASA error types
- **Bates DW et al. (1995)** — *Incidence of Adverse Drug Events* — Statistical grounding
- Feature engineering methodology inspired by clinical NLP research on named entity recognition in discharge summaries
...
To add new drugs to the knowledge base, edit `data/drug_list.txt` (one drug per line). To add new LASA pairs, add rows to `data/processed/drug_pairs.csv` (columns: `drug1, drug2, label`), then retrain with `python modules/03_model_training.py`.
